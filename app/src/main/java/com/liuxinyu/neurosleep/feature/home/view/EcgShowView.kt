...
import com.liuxinyu.neurosleep.util.DensityUtil.dip2px
import com.liuxinyu.neurosleep.util.Utils
...
class EcgShowView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    var SHOW_MODEL = 0
    companion object {
        const val SHOW_MODEL_ALL = 0x00
        const val SHOW_MODEL_DYNAMIC_SCROLL = 0x01
        const val SHOW_MODEL_DYNAMIC_REFRESH = 0x02
    }
    ...
    fun setScale(newScale: Float) {
        scale = newScale
        intervalRowHeart = mWidth / (intervalNumHeart * scale)
        intervalColumnHeart = mHeight / (MAX_VALUE * 2 * scale)
        invalidate()
    }
